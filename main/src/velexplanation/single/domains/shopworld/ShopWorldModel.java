...
public class ShopWorldModel implements FullStateModel {
    ...
    public ShopWorldModel(double hidingEffectiveness, double smallVisibility,
                          double medVisibility, double largeVisibility) {
        ...
        for (ObjectInstance trinket : s.objectsOfClass(CLASS_TRINKET)) {
            ...
            if ((Boolean) trinket.get(VAR_HELD)) {
                ...
                if (size == ShopWorldTrinket.Size.SMALL) {
                    probOfCatching = this.smallVisibility; // Allow the user to set these values
                } else if (size == ShopWorldTrinket.Size.MEDIUM) {
                    probOfCatching = this.medVisibility;
                } else if (size == ShopWorldTrinket.Size.LARGE) {
                    probOfCatching = this.largeVisibility;
                }

                if ((Boolean) trinket.get(VAR_HIDDEN)) {
                    probOfCatching *= (1 - this.hidingEffectiveness);
                }
            }
